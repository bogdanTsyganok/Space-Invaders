...
//    glGetIntegerv(GL_MAX_UNIFORM_LOCATIONS, &max_uniform_location);
    ...
    ::g_pFlyCamera->setEye( glm::vec3(0.0f, -200.0f, -2725.0f) );
...
//    
    ...
    //::g_pTheLights->theLights[1].position = glm::vec4( 0.0f, 500.0f, 0.0f, 1.0f);
    //::g_pTheLights->theLights[1].diffuse = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
    //::g_pTheLights->theLights[1].param1.x = 0.0f;   
    //::g_pTheLights->theLights[1].atten.x = 0.0f;
    //::g_pTheLights->theLights[1].atten.y = 0.001f;
    //::g_pTheLights->theLights[1].atten.z = 0.0001f;
    //::g_pTheLights->theLights[1].param2.x = 1.0f;
    ::g_pTheLights->theLights[1].position = glm::vec4(50.f, 85.f, 320.f, 1.0f);
    ::g_pTheLights->theLights[1].diffuse = glm::vec4(1.0f, 0.0f, 1.0f, 1.0f);
    ::g_pTheLights->theLights[1].param1.x = 0.0f;    // spot
    ::g_pTheLights->theLights[1].atten.y = 0.5f;
    ::g_pTheLights->theLights[1].atten.z = 0.15f;
    ::g_pTheLights->theLights[1].param2.x = 1.0f;

    ::g_pTheLights->theLights[2].position = glm::vec4(-38.f, 117.f, 320.f, 1.0f);
    ::g_pTheLights->theLights[2].diffuse = glm::vec4(0.8f, 0.8f, 0.6f, 1.0f);
    ::g_pTheLights->theLights[2].param1.x = 1.0f;    // spot
    ::g_pTheLights->theLights[2].atten.y = 0.005f;
    ::g_pTheLights->theLights[2].atten.z = 0.00015f;
    ::g_pTheLights->theLights[2].direction = glm::vec4(-16.f, 88.f, 319.f, 1.f) - glm::vec4(-38.f, 117.f, 320.f, 1.f);
    ::g_pTheLights->theLights[2].param2.x = 1.0f;

    ::g_pTheLights->theLights[3].position = glm::vec4(-51.f, 135.f, 355.f, 1.f);
    ::g_pTheLights->theLights[3].diffuse = glm::vec4(0.8f, 0.8f, 0.6f, 1.0f);
    ::g_pTheLights->theLights[3].param1.x = 1.0f;    // spot
    ::g_pTheLights->theLights[3].direction = glm::vec4(48.f, 125.f, 353.f, 1.f) - glm::vec4(-51.f, 135.f, 355.f, 1.f);
    ::g_pTheLights->theLights[3].atten.y = 0.005f;
    ::g_pTheLights->theLights[3].atten.z = 0.00015f;
...
//    ::g_pTheLights->TurnOnLight(1);  // Or this!
    ...
    std::vector<std::string> invaderMeshes;
    invaderMeshes.push_back("SpaceInvader_I1_block_pose_1.ply");
    invaderMeshes.push_back("SpaceInvader_I1_block_pose_2.ply");
    invaderMeshes.push_back("SpaceInvader_I2_block_pose_1.ply");
    invaderMeshes.push_back("SpaceInvader_I2_block_pose_2.ply");
    invaderMeshes.push_back("SpaceInvader_I3_block_pose_1.ply");
    invaderMeshes.push_back("SpaceInvader_I3_block_pose_2.ply");

    cInvaderManager* invaderManager = new cInvaderManager(invaderMeshes);
    ...
    vecModelsToLoad.push_back("SpaceInvader_tank_block.ply");
    vecModelsToLoad.push_back("SpaceInvader_UFO_block.ply");
    vecModelsToLoad.push_back("SpaceInvader_Explosion.ply");
    vecModelsToLoad.push_back("SpaceInvader_Explosion_2.ply");

    cMesh* playerModel = new cMesh();
    playerModel->positionXYZ = ::g_PlayerTankPosition;
    playerModel->meshName = "SpaceInvader_tank_block.ply";
    playerModel->bDontLight = true;
    playerModel->wholeObjectDiffuseRGBA = glm::vec4(0.2f, 0.9f, 0.2f, 1.0f);
    playerModel->friendlyID = 0;
    playerModel->bUseWholeObjectDiffuseColour = true;

    g_vec_pMeshes.push_back(playerModel);
    {
        float initOffset = -900.f;
        for (int k = 0; k < 4; k++)
        {
            cMesh* shieldModel = new cMesh();
            shieldModel->positionXYZ = ::g_PlayerTankPosition;
            shieldModel->meshName = "SpaceInvader_Shield_block.ply";
            shieldModel->bDontLight = true;
            shieldModel->wholeObjectDiffuseRGBA = glm::vec4(0.2f, 0.9f, 0.2f, 1.0f);
            shieldModel->bUseWholeObjectDiffuseColour = true;
            shieldModel->positionXYZ = glm::vec3(initOffset, -900.f, 0.f);
            initOffset += 600.f;
            g_vec_pMeshes.push_back(shieldModel);
        }
    }

    cProjectileManager projectileManager(playerModel, invaderManager);
...
//        ::g_pVAOManager->LoadModelIntoVAO(theModel, program);
    ...
    invaderMesh->meshName = "SpaceInvader_I1_block_pose_1.ply";
    invaderMesh->bDontLight = true;
    invaderMesh->wholeObjectDiffuseRGBA = glm::vec4(1.f, 0.5f, 0.f, 1.f);
...
//        double deltaTime = previousTime - currentTime;        OOOPSS!
        ...
        if (::g_playerInputInfo.z == 0.f)
        {
            playerModel->positionXYZ = ::g_PlayerTankPosition;

            invaderManager->Iterate(deltaTime);

            if (projectileManager.Integrate(deltaTime))
            {
                ::g_playerInputInfo.z = 1.f;
                ::g_vec_pMeshes.erase(std::find(::g_vec_pMeshes.begin(), ::g_vec_pMeshes.end(), playerModel));
            }

            if (::g_playerInputInfo.x == 1.0f)
            {
                projectileManager.PlayerFired();
                ::g_playerInputInfo.y = 0.5f;
                ::g_playerInputInfo.x = 0.f;
            }
            if (::g_playerInputInfo.y >= 0.f)
            {
                ::g_playerInputInfo.y -= deltaTime;
            }
            alienGunRecharge += deltaTime;

            if (alienGunRecharge >= 1.8f)
            {
                alienGunRecharge = 0.f;

                projectileManager.AlienFired();
            }
        }
...
// HACK: Set the transparency for JUST the mobile shipyard...
            ...
            // HACK: Set up a height map from a texture (used in the VERTEX shader)
            {
                ...
                if (pCurrentMesh->friendlyID == 42069)
                {
                    ...
                    glUniform1f(heightMapScale_LocID, 50.f);
